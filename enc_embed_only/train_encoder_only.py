#!/usr/bin/env python3
"""Train embed-only XBridge ablation: NLLB token embeddings -> mapping_embed -> frozen LLM.

Drops the mapping_enc2llm(Enc(x)) path; source feature is mapping_embed(E(x)) only.
"""
...
def save_trainable_checkpoint(model, output_dir: str, config: XBridgeConfig, step: int):
    ...
    embed_state = {k: v.cpu() for k, v in model.mapping_embed.state_dict().items()}
    torch.save(embed_state, os.path.join(ckpt_dir, "mapping_embed.pt"))
    meta = {
        "step": step,
        "trainable": "mapping_embed",
        "llm_only": config.llm_only,
        "use_embed_fusion": getattr(config, "use_embed_fusion", False),
    }
...
def build_model(
    ...
    config = XBridgeConfig(
        ...
        use_embed_fusion=True,
        freeze_enc=True,
        freeze_llm=True,
        freeze_dec=True,
        # Keep False so the forward loss-gate assigns llm_outputs.loss; actual training
        # scope is controlled by requires_grad below (only mapping_embed is unfrozen).
    ...
    model.mapping_embed = model.mapping_embed.to(device)

    for p in model.parameters():
        p.requires_grad = False
    # Embed-only ablation: train mapping_embed only; mapping_enc2llm stays frozen & unused.
    for p in model.mapping_embed.parameters():
        p.requires_grad = True

    if resume_mapping:
        state = torch.load(resume_mapping, map_location="cpu")
        model.mapping_embed.load_state_dict(state, strict=True)
        print(f"Loaded mapping_embed from {resume_mapping}")
    elif reinit_mapping:
        reset_mapping_weights(model.mapping_embed)
        print("Re-initialized mapping_embed from scratch.")
...
def main(
    train_file: str = "",
    output_dir: str = "outputs/enc_mt_embed_only_zh_en_100k",
    mt_path: str = "",
    llm_path: str = "",
    ...
    train_file = train_file or os.path.join(ROOT, "data/encoder_only/opus100_zh_en_100k.jsonl")
