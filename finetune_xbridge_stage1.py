...
def train(
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        input_ids_mt = mt_input_features(
            input_text_m2m=data_point["encoder_input"], 
            source_language=data_point["src_lang"],
            langs_map=langs_map,
            max_length=max_seq_len
        )
        
        # output sentence
        labels = llm_input_features(
            input_texts_llm=f"{data_point['llm_output']}{tokenizer_llm.eos_token}", 
            add_special_tokens=False,
            max_length=max_seq_len
        )
        
        mt_labels = mt_input_features(
            input_text_m2m=data_point["llm_output"], 
            source_language="English",
            langs_map=langs_map,
            max_length=max_seq_len
        )
        
        decoder_input_ids = mt_input_features(
            input_text_m2m=data_point["decoder_output"], 
            source_language=data_point["tgt_lang"],
            langs_map=langs_map,
            max_length=max_seq_len
        )
        
        result["input_ids"] = input_ids_mt + labels
        result["attention_mask"] = [1] * len(result["input_ids"])
        result["augmentation"] = [1] * len(input_ids_mt) + [3] * len(labels)
        result["decoder_input_ids"] = [tokenizer_mt.eos_token_id] + decoder_input_ids
    ...
    if val_set_size > 0:
        ...
        train_data = (
            train_val["train"].map(generate_and_tokenize_prompt, num_proc=16).shuffle()
        )
        val_data = (
            train_val["test"].map(generate_and_tokenize_prompt, num_proc=16).shuffle()
        )
    else:
        train_data = data["train"].map(generate_and_tokenize_prompt, num_proc=16).shuffle()
        val_data = None
